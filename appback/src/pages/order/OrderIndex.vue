<template>
  <div>
    ...
    <div class="stat">
      <span>成功的 总交易额：</span><span class="value">{{stat.total_price}}</span>
      <span>成功率：</span><span class="value">{{stat.total_success_rate}}</span>
      <span>成功单数：</span><span class="value">{{stat.total_success_count}}</span>
      <span>总单数：</span><span class="value">{{stat.total_count}}</span>
    </div>
  ...
  methods: {
    ...
    orderStat: function () {
      this.$network.get(this.$api.order_stat, {...this.query}).then(resp => {
        this.stat.total_price_success = resp.data.total_price_success
        this.stat.total_count_success = resp.data.total_count_success
        this.stat.total_count = resp.data.total_count
      })
    }
  }
}
