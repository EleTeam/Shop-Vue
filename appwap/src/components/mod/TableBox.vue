  ...
  <div class="table-wrap" :class="{scroll:scroll}">
    ...
    <div class="table-header" ref="tableHead">
      <table class="table table-bordered">
        ...
        <tr>
          <th width="80">序号</th>
          <th v-for="(column,index) in columns" v-bind:key="index" :width="column.width">{{column.title}}</th>
        </tr>
        </thead>
      </table>
    </div>
      ...
      <table class="table table-bordered">
        ...
        <slot name="row" v-for="(item,index) in items" :item="item" :index="rowIndex(index)">
          <tr v-bind:key="index">
            ...
            <td v-for="(column,i) in columns" v-bind:key="i">
              {{item[column.field]}}
            </td>
          </tr>
        </slot>
    ...
    <div class="table-page">
      <div class="page-info">
        <span>纪录总数:</span><span class="total">{{total}}</span><span>，共</span><span
          class="total">{{total_page}}</span><span>页，</span>
        <span>每页显示:</span>
      ...
      <div class="page-pagination">
        <ul>
          ...
          <li v-for="(p,i) in pages" v-bind:key="i" :class="{active:p==page}" @click="p!=page&&requestData(p)">{{p}}</li>
          <li class="next" :class="{disabled:page==total_page}" @click="page<total_page&&requestData(page+1)">下一页</li>
          <li class="last" :class="{disabled:page==total_page}" @click="page<total_page&&requestData(total_page)">尾页
          </li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    setPage: function () {
      ...
      let startPage = (this.page - 4) < 1 ? 1 : (this.page - 4)
      let lastPage = (this.page + 4) > this.total_page ? this.total_page : (this.page + 4)
      this.pages = []
          ...
          trs[i].onclick = function (e) {
            if (that.multiSelect) {
              ...
              if (e.shiftKey === 1 && that.getSelectedRowId().length > 0) {
                ...
                let min = isSelected > i ? i : isSelected
                let max = isSelected > i ? isSelected : i
    ...
    getSelectedRowId: function () {
      ...
      for (let i = 0; i < this.select_rows.length; i++) {
        let row = this.items[this.select_rows[i]]
        rowIds.push(row[this.keyName])
      }
      return rowIds
    },
